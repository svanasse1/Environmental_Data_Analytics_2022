...
```{r}
fahr.temp.high <- c(44, 48, 34, 43, 51, 50, 59, 65, 59, 44); fahr.temp.high
fahr.temp.low <- c(27, 26, 15, 25, 25, 33, 48, 46, 26, 22); fahr.temp.low
```
...
```{r}
fahr.to.cel <- function(x){
  temp <- (x-32)*(5/9)
  return(temp)
}

cel.temp.high <- fahr.to.cel(fahr.temp.high); cel.temp.high 
cel.temp.low <- fahr.to.cel(fahr.temp.low); cel.temp.low
```

Combine your four vectors into a data frame and add informative column names.

```{r}
temp.dataframe <- cbind(fahr.temp.high,fahr.temp.low,cel.temp.high,cel.temp.low )
temp.dataframe <- data.frame("Fahrenheit High"=fahr.temp.high,"Fahrenheit Low"=fahr.temp.low,"Celsius High"=cel.temp.high,"Celsius Low"=cel.temp.low)
temp.dataframe
```
...
```{r}
summary(temp.dataframe)
sd(temp.dataframe$Fahrenheit.High)
sd(temp.dataframe$Fahrenheit.Low)
sd(temp.dataframe$Celsius.High)
sd(temp.dataframe$Celsius.Low)
```

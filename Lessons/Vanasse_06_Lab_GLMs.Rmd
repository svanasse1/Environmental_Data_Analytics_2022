...
Ozone.Density.Plot <- 
  ...
  labs(y = "Density", x = "Ozone AQI value", fill = "") 
...
## Dependent variable ~ Independent variable --> this is important since we are trying to predict PM2.5 from Ozone, so PM2.5 is the dependent variable
regression.1 <- lm(data = subset(EPAair), 
                   PM2.5 ~ Ozone)
...
AIC <- lm(data = EPAair.subset, PM2.5 ~ Ozone + meanLat + meanLng + Month + 
                    Year)
step(AIC)
...
Ex7.regression <- lm(data = EPAair, PM2.5 ~ Ozone + Month + meanLat + meanLng)
summary(Ex7.regression)
```
